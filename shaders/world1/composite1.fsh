...
#include "/Lib/TimeArray.glsl"
    ...
    vec3 lightScattering(vec3 fragpos, vec2 lightPos, vec3 color, vec3 sunColor) {
        ...
        // Volumetric Light
            ...
            #define VL_RENDERDISTANCE 50.0 // [10.0 20.0 30.0 40.0 50.0 60.0 70.0 80.0 90.0 100.0 110.0 120.0 130.0 140.0 150.0 160.0 170.0 180.0 190.0 200.0]
            ...
            if (isEyeInWater == 1) vlRenderQuality *= 0.5;
...
void main() {
  ...
  bool water = mat > 0.09 && mat < 0.11;
    ...
    vec2 refraction = vec2(0.0);
        ...
        float	refractionSpeed	= END_HEATWAVE_SPEED;
        float refractionSize = END_HEATWAVE_SIZE;
