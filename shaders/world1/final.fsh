...
vec3 renderDOF(vec3 color, float depth) {
    ...
    const float maxDepthRange = 0.8; // Adjust this for a more realistic DoF range
    
    // Calculate the focus distance and factor based on depth
    float focus = depth - centerDepthSmooth;
    float factor = clamp(focus * blurFactor, -maxBlurFactor, maxBlurFactor);

    // Prevent DoF effect for the hand (close objects)
    bool hand = depth < 0.56;
    if (hand) factor = 0.0;

    // Correct aspect ratio for horizontal and vertical offsets
    vec2 aspectCorrect = vec2(1.0, aspectRatio);

    // Sample offsets for blur (4 cardinal directions)
    vec2 offsets[4] = vec2[4](vec2(1.0, 0.0), vec2(0.0, 1.0), vec2(-1.0, 0.0), vec2(0.0, -1.0));

    // Variable to store the accumulated blur samples
    vec3 blurSamples = vec3(0.0);

    // Perform 4x4 blur sampling
    for (int i = 0; i < 4; i++) {
