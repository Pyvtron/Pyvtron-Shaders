...
uniform float gbufferProjection;
...
vec3 pathTraceReflection(vec3 fragpos, vec3 normal, vec3 color, int depth) {
    if (depth <= 0) return color;

    vec3 reflectedDir = reflect(normalize(fragpos), normal);
    vec3 reflectedPos = fragpos + reflectedDir * 0.1; // Offset to avoid self-intersection

    vec4 reflectedPosWorld = gbufferModelViewInverse * vec4(reflectedPos, 1.0);
    vec4 reflectedPosProj = gbufferProjection * reflectedPosWorld;
    reflectedPosProj /= reflectedPosProj.w;
    vec2 screenCoord = reflectedPosProj.xy * 0.5 + 0.5;

    if (screenCoord.x < 0.0 || screenCoord.x > 1.0 || screenCoord.y < 0.0 || screenCoord.y > 1.0) {
        return color;
    }

    vec3 reflectedColor = texture2D(colortex0, screenCoord).rgb * MAX_COLOR_RANGE;
    vec3 reflectedNormal = decodeNormal(texture2D(colortex1, screenCoord).yz);

    return mix(color, pathTraceReflection(reflectedPos, reflectedNormal, reflectedColor, depth - 1), 0.5);
}
...
void main() {
    ...
    #include "/Lib/Colors.glsl"
        ...
        if (isEyeInWater == 0) { 
            ...
            vec3 eyeDir = normalize(cameraPosition - fragposition0.xyz);
            vec3 reflectedDir = reflect(eyeDir, waterNormal);
            vec3 reflectedPos = fragposition0.xyz + reflectedDir * 0.1; 
